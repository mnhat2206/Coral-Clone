...
function CategoryForm() {
    ...
    useEffect(() => {
        if (params.categoryId) {
            fetch(`http://localhost:3002/api/category/categoryParent?id=${params.categoryId}`)
                .then((res) => res.json())
                .then((data) => {
                    setOptions(
            ...
            fetch(`http://localhost:3002/api/category/categoryParent`)
                .then((res) => res.json())
                .then((data) => {
                    setOptions(
                        data.categoryParentOptions.map((category) => {
    ...
    const handleCreate = async () => {
        ...
        fetch('http://localhost:3002/api/categories', options)
            .then(() => {
                setCategory({
                    name: '',
                    isActive: true,
    ...
    const handleUpload = async () => {
        ...
        fetch(`http://localhost:3002/api/categories/${id}`, options)
            .then(() => {
